...
async def _bounded_wait(awaitable: Awaitable[object], *, timeout: float = 0.5) -> None:
    task = asyncio.create_task(awaitable)
    try:
        done, _ = await asyncio.wait({task}, timeout=timeout)
        if task not in done:
            raise AssertionError("waitable-did-not-finish")
        task.result()
    finally:
        if not task.done():
            task.cancel()
...
async def _bounded_until(predicate: Callable[[], object], *, timeout: float = 0.5) -> None:
...
def _patch_shared_cleanup_probe_bounds(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(supervisor_module, "_PROVIDER_CLOSE_SECONDS", 0.16)
    monkeypatch.setattr(supervisor_module, "_PROVIDER_JOIN_SECONDS", 0.18)
    monkeypatch.setattr(supervisor_module, "_CLEANUP_ACK_SECONDS", 0.2)
    monkeypatch.setattr(supervisor_module, "_CLEANUP_TOTAL_SECONDS", 0.3)


def _record_receipt_wait_timeouts(monkeypatch: pytest.MonkeyPatch) -> list[float]:
    receipt_wait_timeouts: list[float] = []
    real_wait_for = supervisor_module.asyncio.wait_for

    async def record_receipt_wait_timeout(
        awaitable: Awaitable[object],
        timeout: float | None = None,
    ) -> object:
        if timeout is not None:
            receipt_wait_timeouts.append(float(timeout))
        return await real_wait_for(awaitable, timeout=timeout)

    monkeypatch.setattr(supervisor_module.asyncio, "wait_for", record_receipt_wait_timeout)
    return receipt_wait_timeouts


def _record_observe_deadlines(
    monkeypatch: pytest.MonkeyPatch,
) -> list[tuple[str, float, float]]:
    observed: list[tuple[str, float, float]] = []
    original_observe = CorePttSessionSupervisor._observe_task_until

    async def record_observe(
        self: CorePttSessionSupervisor,
        task: asyncio.Task[Any],
        *,
        deadline: float,
        cancelled_ok: bool,
    ) -> bool:
        observed.append((task.get_name(), deadline, asyncio.get_running_loop().time()))
    ...
    monkeypatch.setattr(CorePttSessionSupervisor, "_observe_task_until", record_observe)
    return observed


def _install_delayed_cleanup_adoption(
    monkeypatch: pytest.MonkeyPatch,
) -> tuple[asyncio.Event, asyncio.Event]:
    cleanup_adopted = asyncio.Event()
    release_cleanup = asyncio.Event()
    original_cleanup = CorePttSessionSupervisor._cleanup_despite_cancellation
    ...
    async def delayed_cleanup(
        ...
        *,
        ...
        return await original_cleanup(self, failure, first=first)  # type: ignore[arg-type]

    monkeypatch.setattr(
        CorePttSessionSupervisor,
        "_cleanup_despite_cancellation",
        delayed_cleanup,
    )
    return cleanup_adopted, release_cleanup
...
async def test_cleanup_receipt_wait_uses_one_shared_t0_deadline(
    ...
    monkeypatch.setattr(supervisor_module, "_CLEANUP_TOTAL_SECONDS", 0.8)
    receipt_wait_timeouts: list[float] = []
    real_wait_for = supervisor_module.asyncio.wait_for
    ...
    assert outcome is PttSessionOutcome.CLEANUP_INCOMPLETE
    assert cancellation.calls == 1
    assert len(receipt_wait_timeouts) == 1
    assert 0.0 < receipt_wait_timeouts[0] < 0.5


@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("first_control", "later_control", "expected_outcome", "expected_reason"),
    (
        (
    ...
    ids=("stop", "cancel", "error"),
)
async def test_edge_cleanup_trigger_t0_is_sampled_before_guard_lock_wait(
    monkeypatch: pytest.MonkeyPatch,
    first_control: PttControl,
    later_control: PttControl,
    expected_outcome: PttSessionOutcome,
    expected_reason: PttErrorReason,
) -> None:
    _patch_shared_cleanup_probe_bounds(monkeypatch)
    clock = _Clock()
    bridge = _Bridge(mode=PttInputMode.REACHY_LOCAL)
    pipeline = _BlockingPipeline(clock)
    supervisor = _supervisor(
    ...
    run = asyncio.create_task(supervisor.run(TURN_ID))
    await asyncio.wait_for(pipeline.entered.wait(), timeout=1)
    receipt_wait_timeouts = _record_receipt_wait_timeouts(monkeypatch)
    await supervisor._guard_lock.acquire()  # noqa: SLF001 - held-lock T0 proof
    try:
        bridge._edge_control(first_control)  # noqa: SLF001 - valid cleanup trigger
        await _bounded_until(lambda: bridge.incoming.empty(), timeout=1)
        await asyncio.sleep(0.12)
        bridge._edge_control(later_control)  # noqa: SLF001 - must not replace first trigger
        bridge._receipt()  # noqa: SLF001 - complete cleanup handshake
    finally:
        supervisor._guard_lock.release()  # noqa: SLF001 - held-lock T0 proof

    assert await _bounded_outcome(run, timeout=1) is expected_outcome
    assert len(receipt_wait_timeouts) == 1
    assert 0.0 < receipt_wait_timeouts[0] < 0.12
    abort = next(
        frame
        for frame in bridge.sent
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("first_control", "expected_outcome"),
    (
        (PttControl.stop(TURN_ID), PttSessionOutcome.CANCELLED),
        (PttControl.cancel(TURN_ID), PttSessionOutcome.CANCELLED),
        (
            PttControl.error(TURN_ID, PttErrorReason.PROVIDER_FAILED),
            PttSessionOutcome.PROVIDER_FAILED,
        ),
...
async def test_accepted_edge_cleanup_observation_caps_later_hard_stop_provider_deadline(
    monkeypatch: pytest.MonkeyPatch,
    first_control: PttControl,
    expected_outcome: PttSessionOutcome,
) -> None:
    _patch_shared_cleanup_probe_bounds(monkeypatch)
    observed_deadlines = _record_observe_deadlines(monkeypatch)
    clock = _Clock()
    bridge = _Bridge(mode=PttInputMode.REACHY_LOCAL)
    cancellation = _BlockingCancellation()
    pipeline = _BlockingPipeline(clock)
    ...
    try:
        await asyncio.wait_for(pipeline.entered.wait(), timeout=1)
        await asyncio.wait_for(supervisor._guard_lock.acquire(), timeout=1)  # noqa: SLF001
        try:
            observed_before_lock_wait = asyncio.get_running_loop().time()
            bridge._edge_control(first_control)  # noqa: SLF001 - valid cleanup trigger
            await _bounded_until(lambda: bridge.incoming.empty(), timeout=1)
            await asyncio.sleep(0)
            await asyncio.sleep(0.12)
            hard_stop_at = asyncio.get_running_loop().time()
            supervisor._set_signal(supervisor_module._Signal.ADAPTER_FAILED)  # noqa: SLF001
            await _bounded_wait(cancellation.entered.wait(), timeout=1)
            await _bounded_until(
                lambda: any(name == "core-ptt-provider-close" for name, _, _ in observed_deadlines),
                timeout=1,
            )
        finally:
            supervisor._guard_lock.release()  # noqa: SLF001

        await _bounded_until(lambda: supervisor._cleanup_outcome_latched, timeout=1)  # noqa: SLF001
        provider_close = next(
            record for record in observed_deadlines if record[0] == "core-ptt-provider-close"
        )
        _, provider_deadline, provider_observed_at = provider_close

        assert supervisor._requested_outcome is expected_outcome  # noqa: SLF001
        assert supervisor._cleanup_trigger_t0 is not None  # noqa: SLF001
        assert supervisor._cleanup_trigger_t0 < hard_stop_at  # noqa: SLF001
        assert provider_deadline - provider_observed_at < 0.08
        assert supervisor._cleanup_loop_deadline <= observed_before_lock_wait + 0.35  # noqa: SLF001

        cancellation.release.set()
        assert await _bounded_outcome(run, timeout=1) is PttSessionOutcome.CLEANUP_INCOMPLETE
        assert supervisor._signal is supervisor_module._Signal.ADAPTER_FAILED  # noqa: SLF001
    finally:
        cancellation.release.set()
        if not run.done():
...
@pytest.mark.asyncio
async def test_accepted_edge_cleanup_observation_caps_later_hard_stop_bridge_deadline(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(supervisor_module, "_PROVIDER_CLOSE_SECONDS", 0.16)
    monkeypatch.setattr(supervisor_module, "_PROVIDER_JOIN_SECONDS", 0.18)
    monkeypatch.setattr(supervisor_module, "_CLEANUP_ACK_SECONDS", 0.2)
    monkeypatch.setattr(supervisor_module, "_CLEANUP_TOTAL_SECONDS", 0.3)
    monkeypatch.setattr(supervisor_module, "_BRIDGE_CLOSE_SECONDS", 1.0)
    clock = _Clock()
    bridge = _HangingCloseBridge(mode=PttInputMode.REACHY_LOCAL)
    pipeline = _BlockingPipeline(clock)
    supervisor = _supervisor(
    ...
    try:
        ...
        try:
            observed_before_lock_wait = asyncio.get_running_loop().time()
            bridge._edge_control(PttControl.stop(TURN_ID))  # noqa: SLF001 - valid cleanup trigger
            await _bounded_until(lambda: bridge.incoming.empty(), timeout=1)
            await asyncio.sleep(0)
            await asyncio.sleep(0.12)
            hard_stop_at = asyncio.get_running_loop().time()
            supervisor._set_signal(supervisor_module._Signal.ADAPTER_FAILED)  # noqa: SLF001
            await _bounded_wait(bridge.close_entered.wait(), timeout=1)
        finally:
            supervisor._guard_lock.release()  # noqa: SLF001

        await _bounded_until(lambda: supervisor._cleanup_outcome_latched, timeout=1)  # noqa: SLF001
        await _bounded_until(lambda: hasattr(supervisor, "_cleanup_loop_deadline"), timeout=1)

        assert supervisor._cleanup_trigger_t0 is not None  # noqa: SLF001
        assert supervisor._cleanup_trigger_t0 < hard_stop_at  # noqa: SLF001
        assert supervisor._cleanup_loop_deadline <= observed_before_lock_wait + 0.35  # noqa: SLF001
        assert await _bounded_outcome(run, timeout=1) is PttSessionOutcome.CLEANUP_INCOMPLETE
        assert bridge.close_cancellations >= 1
        assert bridge.close_finalized.is_set()
    finally:
...
@pytest.mark.asyncio
@pytest.mark.parametrize("poison_kind", ("wrong_sequence", "wrong_turn"))
async def test_rejected_edge_cleanup_frame_does_not_reuse_presampled_t0_for_poison(
    monkeypatch: pytest.MonkeyPatch,
    poison_kind: str,
) -> None:
    _patch_shared_cleanup_probe_bounds(monkeypatch)
    ...
    try:
        await asyncio.wait_for(pipeline.entered.wait(), timeout=1)
        await supervisor._guard_lock.acquire()  # noqa: SLF001 - held-lock rejection proof
        try:
            if poison_kind == "wrong_sequence":
                poisoned_cleanup = encode_control_frame(
                    sequence=bridge.edge_sequence + 1,
                    control=PttControl.stop(TURN_ID),
                )
            else:
                poisoned_cleanup = encode_control_frame(
                    sequence=bridge.edge_sequence,
                    control=PttControl.stop(UUID("30000000-0000-4000-8000-0000000000ff")),
                )
            observed_before_lock_wait = asyncio.get_running_loop().time()
            bridge.incoming.put_nowait(poisoned_cleanup)
            await _bounded_until(lambda: bridge.incoming.empty(), timeout=1)
            await asyncio.sleep(0)
            await asyncio.sleep(0.06)
            release_at = asyncio.get_running_loop().time()
        finally:
            supervisor._guard_lock.release()  # noqa: SLF001 - held-lock rejection proof

        assert await _bounded_outcome(run, timeout=1) is PttSessionOutcome.CLEANUP_INCOMPLETE
        assert supervisor._signal is supervisor_module._Signal.PROTOCOL_POISONED  # noqa: SLF001
        assert supervisor._cleanup_trigger_t0 is not None  # noqa: SLF001
        assert supervisor._cleanup_trigger_t0 >= release_at  # noqa: SLF001
        assert supervisor._cleanup_trigger_t0 - observed_before_lock_wait >= 0.05  # noqa: SLF001
...
@pytest.mark.asyncio
@pytest.mark.parametrize("branch", ("first", "pre_capture", "pre_submit"))
async def test_capture_phase_terminal_cancel_latches_t0_before_session_failure(
    monkeypatch: pytest.MonkeyPatch,
    branch: str,
) -> None:
    _patch_shared_cleanup_probe_bounds(monkeypatch)
    cleanup_adopted, release_cleanup = _install_delayed_cleanup_adoption(monkeypatch)
    clock = _Clock()
    bridge = _Bridge(
        mode=PttInputMode.CORE_TERMINAL_TOGGLE,
        rapid_submit=branch == "pre_capture",
    )
    initial_events = {
        "first": (CorePttEvent.CANCEL,),
        "pre_capture": (CorePttEvent.START, CorePttEvent.CANCEL),
        "pre_submit": (CorePttEvent.START,),
    }[branch]
    input_port = _Input(initial_events)
    supervisor = _supervisor(
        mode=PttInputMode.CORE_TERMINAL_TOGGLE,
        clock=clock,
        bridge=bridge,
    ...
    run = asyncio.create_task(supervisor.run(TURN_ID))
    try:
        if branch == "pre_submit":
            await _bounded_until(lambda: bridge.capture_started, timeout=1)
            input_port.put(CorePttEvent.CANCEL)
        await _bounded_wait(cleanup_adopted.wait(), timeout=1)
        receipt_wait_timeouts = _record_receipt_wait_timeouts(monkeypatch)
        await asyncio.sleep(0.12)
        release_cleanup.set()

        assert await _bounded_outcome(run, timeout=1) is PttSessionOutcome.CANCELLED
        assert len(receipt_wait_timeouts) == 1
        assert 0.0 < receipt_wait_timeouts[0] < 0.12
        abort = next(
            frame
...
@pytest.mark.parametrize(
    "first_control",
    (PttControl.stop(TURN_ID), PttControl.cancel(TURN_ID)),
    ids=("stop", "cancel"),
)
async def test_edge_stop_or_cancel_receipt_deadline_uses_first_trigger_t0_after_delay(
    monkeypatch: pytest.MonkeyPatch,
    first_control: PttControl,
) -> None:
    monkeypatch.setattr(supervisor_module, "_PROVIDER_CLOSE_SECONDS", 0.16)
    ...
    bridge._edge_control(first_control)  # noqa: SLF001 - deterministic cleanup trigger
    await _bounded_wait(cleanup_adopted.wait(), timeout=1)
    await asyncio.sleep(0.12)
    bridge._edge_control(  # noqa: SLF001 - later cleanup signal must not reset T0
        PttControl.error(TURN_ID, PttErrorReason.PROVIDER_FAILED)
    )
...
@pytest.mark.asyncio
async def test_terminal_privacy_cancel_receipt_deadline_uses_first_trigger_t0_after_delay(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(supervisor_module, "_PROVIDER_CLOSE_SECONDS", 0.16)
    monkeypatch.setattr(supervisor_module, "_PROVIDER_JOIN_SECONDS", 0.18)
