...
def test_synthetic_capability_report_is_strict_contract_and_sanitized() -> None:
    ...
    assert ReachyCapabilityEvidenceV1 is CapabilityReport
    assert report.schema_version == "tuntun.reachy-capability-report.v1"
    assert report.source == "synthetic"
    assert report.probe_version == "0.1.0"
    assert report.sdk_version == "0.0.0"
    assert report.daemon_version == "0.0.0"
    assert report.input_rate_hz == 16000
    assert report.input_channels == 1
    assert report.output_rate_hz == 16000
    ...
    assert "input_media" not in rendered
    assert "output_media" not in rendered
    ...
    with pytest.raises(ValidationError):
        CapabilityReport.model_validate({**raw, "input_media": {"sample_rate_hz": 16000}})
