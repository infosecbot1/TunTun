...
Tuntun is a local-authority, bilingual family-assistant framework designed around a Reachy Mini Wireless, an owner-approved local Mac, and explicitly bounded integrations for home automation, cameras, media, private AI, desktop assistance, robotics, and remote access.
...
3. Begin with the approved-host and delivered-Reachy inventory, then the isolated 10-working-day voice POC in [deliverable S](docs/superpowers/specs/2026-08-27-tuntun-program-assurance-delivery-i-s.md#s-first-proof-of-concept-plan--mac--reachy-voice-loop).
...
- Owner-approved Darwin arm64 Mac as the active Phase 1 local authority host; Intel macOS remains a mandatory distribution target, not active household evidence.
...
- Archer BE800 outer/primary router with downstream ASUS GT-AX6000 and three AX5400 AiMesh nodes. The active Core host is the owner-approved Darwin arm64 Mac and, for the family-ready baseline, is single-homed on the trusted ASUS/AiMesh LAN with any direct BE800 cable disconnected while Tuntun runs. No public inbound service is allowed; any later dual-homed mode requires its separate fail-closed qualification.
...
The active Phase 1 development and household-validation Core host is the owner-approved Darwin arm64 Mac defined in [ADR 0001](docs/architecture/decisions/0001-phase1-host-baseline.md). Intel macOS remains mandatory supported-distribution CI evidence, and moving household deployment back to the 2020 Intel Mac requires a fresh real-host qualification.
