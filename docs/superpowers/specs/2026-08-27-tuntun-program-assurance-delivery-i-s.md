...
|---|---:|---|---|
| Intel MacBook Pro 2020, 16 GB RAM | 1 | Always-home Tuntun Core, owner API, initial recorder, bounded local utilities | FileVault, sleep/power, thermal, network, SQLCipher, SSD, and long-soak gates |
...
| TP-Link Archer BE800 | 1 | Primary router connected to ISP ONT; outer office-laptop network | No public forwarding/UPnP; two-router path and actual firmware documented |
...
| P5 | Higher-capacity local-inference workstation, approximately 24 GB usable accelerator memory | 0 or 1 | Provisional S$3,500–7,500 landed | Preferred Phase 5 planning tier; no purchase until named-task quality, Hinglish, safety, latency, power, noise, update, and 3-year TCO beat the cloud/Intel baseline |
...
|---|---|---|---|
| Operating systems | Current supported macOS on the owned Intel Mac; Home Assistant OS on Green; vendor-supported Reachy OS/daemon; minimal pinned Linux only on later room/display/inference/robot appliances | Linux on the Mac; Windows host; one general-purpose NAS/VM OS for every service | Reuses owned/supported hardware and keeps deterministic HA separate; every exact OS/build is evidence-bound, patched, rollback-tested, and cannot silently change a trust boundary |
| Process and container isolation | Native supervised Mac processes for the canonical monolith; dedicated accounts/sandboxes for recorder, desktop, and plugins; OCI containers only for a proved disposable D4 job or separately isolated appliance | Docker Compose/Kubernetes for the whole home; one privileged container; no process isolation | Avoids a container control plane on the 16 GB Mac while using real process/key boundaries where raw media, code, or third-party plugins demand them; a container alone is never treated as a security sandbox |
...
| STT/reasoning/TTS | Initial OpenAI route: GPT Transcribe → local policy/context → GPT-5.6 Sol → GPT-4o Mini TTS; Qwen3.7 Plus disabled/evaluation-only fallback | Fully local Whisper/Piper/small LLM; Qwen Max; vendor assistants | Quality-first bilingual baseline within S$100 soft/S$150 hard cap; Phase 5 migrates tasks only after evidence; provider results remain proposals |
...
1. Verify supported Intel macOS version, FileVault, disk reserve, time synchronization, owner account, no public forwarding/UPnP, and network topology.
...
| `CONTAINED_REMOTE` | VPN routes/sessions removed; local household use continues | Lost remote device, VPN/ACL/firewall/cert drift | Local owner passkey after revoke/rotation/scan |
...
| 5 | Public/remote network exposure or stolen owner device | M | H | No forwarding/Funnel/subnet route; VPN + app passkey; scans; local-only high-impact actions | Unexpected listener/route, lost device, ACL/Tailnet Lock/cert drift | Owner + remote/security maintainer | P1–P6 |
...
| Mac remains canonical control host | Existing always-home Intel hardware is sufficient for policy/storage/orchestration | Single host needs power, backup, resource, and recovery controls; not frontier local LLM | Reliability or measured resource envelope fails |
...
| TVs start `DISPLAY_ONLY_MANUAL` | Model names do not prove local API, CEC, IR, or observation | User selects input/power until exact-unit evidence | Each TV passes its own capability and recovery gate |
...
The first POC answers one question: **Can the actual Reachy Mini Wireless and 2020 Intel Mac deliver a responsive, interruptible English/Hindi/Hinglish “Hello Tuntun” voice turn through the selected cloud route without retaining raw household conversation data?**
