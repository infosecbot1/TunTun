...
| Initial scope | Family voice assistant only; no lights, Tuya, Reolink, Home Assistant, TV, or multi-room control |
...
| Primary AI route | GPT Transcribe → local policy/context → GPT-5.6 Sol → GPT-4o Mini TTS |
...
| Delivery horizon | Two-stage private-beta program: disposable voice loop in weeks 1–2, then family-ready private-beta target during the following 4–6 weeks; C0/C1 public-release hardening is a subsequent evidence-driven track |
...
| Web modes | Controlled cited lookup is the household default; no-web is always available; experimental multi-pass research is owner-only, session-scoped, search-only, isolated, and disabled by default |
...
Tuntun has one recognizable persona with profile-specific delivery rules:

| Profile class | Default delivery |
|---|---|
| Owner/security architect | Precise, technically deep, explicit assumptions and security trade-offs |
| Adult household partner | Practical, concise, task-oriented household guidance |
| K2 child | Short sentences, age-appropriate explanations, gentle questions, no private-adult disclosure |
| N1 child | Very short, warm, concrete language; strong child-safety constraints |
| Guest | Helpful general answers without private memory retrieval or personalized permissions |
...
- Every memory has one closed `audience`: `subject_private`, `guardian_child`, `household_adults`, or `household_all`. Adult memories default to `subject_private`. Marking content `household_all` is an explicit approved decision; “household” sensitivity alone never makes it child-readable.
- A child may retrieve only that child's approved `subject_private`/`guardian_child` memories and explicitly child-safe `household_all` memories. A child can never retrieve `household_adults`, another subject's namespace, an adult's `subject_private` memory, policy memory, or a record whose guardian/consent binding is stale. Guest retrieves none.
...
- Home Assistant, Tuya/Zigbee light control, Reolink cameras, Reolink Home Hub Pro, and NAS integration.
...
The edge process has no OpenAI/Qwen key, no Mac database key, no family memory, and no durable biometric template. It sends audio only after wake detection. Camera frames are sampled only for explicit enrollment, an active conversation identity window, or a current re-encounter confirmation performed physically at Reachy. It never writes routine frames, crops, or audio to disk and never exposes a candidate video stream to the browser.
...
- The edge initiates an outbound `wss` connection to the Mac’s dedicated LAN edge gateway on port 7443.
...
- A two-second missed-heartbeat threshold puts Reachy into offline-essential mode. Reconnection never resumes old speech or movement; a new wake is required.
- Commissioning restricts daemon/media/API/WebRTC ports to loopback with the robot firewall where the delivered firmware safely permits it. Otherwise production identity/camera requires an isolated VLAN plus explicit residual-risk acceptance. A detected competing controller immediately closes media, stops motion/playback, and enters owner-recoverable error-safe.
...
- **Experimental multi-pass research mode** is disabled by default and requires an owner passkey plus explicit per-session activation. It uses a fresh isolated context with no family-memory retrieval, no cookies or authenticated sessions, no filesystem access, no downloads, no code execution, no form submission, and no private/LAN address access. It expires at session end or after 30 minutes, whichever is earlier.
...
- Search query bodies and page contents are ephemeral. Durable state contains only content-minimized route, provider, domain, timing, cost, decision, and citation-commitment receipts; URLs shown to the user may appear in an owner-requested export but not ordinary logs.
...
8. The final transcript remains only in a process-local ephemeral turn context. The language tracker labels English, Hindi, or Hinglish and records the current switch behavior.
...
13. Search output is treated as hostile data: returned URLs resolving to local/private/special-use addresses are rejected, Tuntun performs no local follow-up fetch, result bodies are normalized and size-capped, injection/content checks run, and turn-scoped citation references are issued. Search-assisted reasoning uses an answer-and-citations-only schema with no action/memory intent fields. Failure produces either an explicit no-web answer path or a bounded inability response; it never silently fabricates freshness.
...
18. A second DLP, sensitivity, citation, and TTS-consent scan validates the answer. The budget service reserves worst-case TTS cost before the adapter requests explicit 24 kHz signed-16-bit little-endian mono PCM from GPT-4o Mini TTS. Audio is converted to the probed Reachy playback format.
...
20. Only an ordinary non-search turn can send derived memory claims into the local proposal policy. Working state and content-minimized audit are automatic; other writes follow the approval matrix. A search-assisted turn cannot derive a proposal; acting on or remembering a searched fact requires a later ordinary turn and fresh validation without reusing the prior turn's citation authority.
...
- Passive discovery is release-default off. Enabling it is the closed high-risk `identity.discovery.enable` action and requires a fresh owner passkey bound to the current policy, common-area configuration, notice version, retention limit, and candidate-encryption root. It is limited to configured common-area Reachy placement and requires a conspicuous household/visitor notice plus an owner “visitors expected” off switch. Unknown child-like/age-uncertain candidates are never retained. This remains a documented residual-consent risk; deployments unable to provide notice keep the feature disabled. `identity.discovery.opt_out` remains an immediate privacy-enhancing operation and never waits for step-up.
- Unknown candidates store only encrypted embeddings, never images/crops; expiry is no later than seven days.
- Review is offered only when the candidate is currently encountered again and the owner confirms physically at Reachy; the browser receives presence/quality state, not a camera stream or stored portrait.
- Unknown candidates gain no permission and remain Guest until explicit enrollment completes.
...
- Biometric templates, unknown candidates, memory embeddings, and recovery-sensitive values use random per-record data-encryption keys (DEKs) wrapped by purpose-specific roots. Deletion destroys the record/wrapped DEK and makes ciphertext immediately inaccessible; it does not overclaim physical erasure from SSD media.
...
| Unknown face/voice candidate | Maximum seven days; dismissal makes it immediately inaccessible, followed by bounded purge |
...
- `gpt-4o-mini-tts` receives second-pass validated answer text. Text is sentence-segmented below its 2,000-token input limit; `response_format="pcm"` explicitly returns 24 kHz signed-16-bit little-endian mono, which is converted to the probed Reachy playback format.
- Provider errors never log request/response bodies.

### 12.2 Qwen route

- Qwen3.7 Plus is installed disabled.
- It may be evaluated only on at least 240 synthetic/de-identified cases covering the three language modes and four profile classes.
- It may become a fallback only for public/household, non-child-identifying, non-biometric, non-secret, low-sensitivity turns after the owner accepts the evaluation report.
...
- Local code validates the response schema, source count, URL scheme/domain/public-address resolution, redirect chain, text size, and issued citation commitments before a second no-search reasoning call may see normalized excerpts. This separation prevents search content from directly producing a trusted internal proposal.
...
- The answer must cite only turn-issued sources. If source inclusion is absent, citation validation fails, or the provider/account does not support the required controls, Tuntun falls back to no-web behavior rather than silently presenting an unverified current answer.
...
2. **Approvals:** memory proposals, identity candidates, sensitive changes, and budget decisions.
3. **People & identity:** profiles, consent, enrollment, calibration status, candidate dismissal, and revocation.
...
Candidate review displays only that a candidate is currently present at Reachy and requires physical confirmation; no live camera stream is proxied to the browser. High-risk exports, deletion, restores, provider/bind changes, and enrollment use action-bound step-up plus idempotency. Export/download streams are authenticated, short-lived, no-store, safely named, and never written under the static-asset path.

Privacy Shield must be one prominent action. Activating it stops playback/motion, blocks cloud requests and media egress, pauses camera processing, cancels the active graph, conservatively settles potentially billable provider attempts and releases only provably unsent reservations, clears graph/ephemeral working context, and updates Reachy and UI state. The UI presents microphone listening, camera processing, and cloud transmission as separate facts; it does not show a misleading composite “secure” score.

### 13.1 Operating and maintenance modes

- **Household mode** is the supported default and targets less than 30 minutes of routine owner maintenance per week. Health checks, retention, encrypted backups, integrity verification, stale-session cleanup, provider-review expiry, and bounded self-recovery run automatically. The console explains failures and offers safe recovery actions.
...
- Feature updates are never silent. The system verifies a signed release manifest, creates and verifies an encrypted pre-update backup, shows the owner the version/security notes, requires approval, health-checks the new version, and rolls back code plus schema atomically on failure. Critical-update notifications may be automatic; installation is owner-approved.
...
| `C0` | Owner may approve a public candidate | Every mandatory quantitative gate in Sections 16.1–16.6; signed feature-manifest evidence for each optional feature; signed security evidence; 500-turn run; two eight-hour soaks; staged household trial; fresh-Mac recovery; and no unmitigated high/critical finding |
| `C1` | Public Apache-2.0 beta publication | Candidate is bit-for-bit bound to approved C0 evidence; signed package, rollback, SBOM, licences/notices, reproducible CI artifacts, and clean-install verification |
...
- owner, adult-partner, K2, N1, and Guest policy classes; guarded-child enforcement; explicit non-biometric profile selection; and Guest isolation on every uncertain/conflicting identity path;
...
The following may remain visibly disabled at `FB0` without weakening that gate: automatic face/voice personalization if calibration or presentation-attack evidence fails (explicit profile selection plus Guest remains mandatory); Qwen routing; passive identity discovery; Realtime speech; LAN console mode; experimental multi-pass research; and advanced-owner/developer features whose isolation tests have not passed. These are not silently simulated or described as available. At C0, a signed feature manifest marks each optional capability `enabled` or `absent`. Enabled features must pass every applicable Section 16 gate. An absent feature must pass negative reachability, route/registration, UI/CLI/API-advertising, configuration, packaging, and clean-install tests proving it cannot be invoked or mistaken for available. Thus removal is evidence, not an exemption from C0.
...
- Unknown candidates expire within seven days and dismissed candidates become immediately inaccessible, with purge/no-resurrection tests.
...
- Prompt-injection, malicious citation, redirect, DNS-rebinding/private-address, oversized result, unsupported scheme, source-reuse, and cross-turn citation tests cannot create an action/memory proposal or an uncited current-information claim.
...
- C0/C1 update tests reject a wrong signer, replayed or downgraded manifest, mismatched artifact digest, failed backup verification, incompatible schema, and failed post-update health check. Every failure leaves the prior signed version and schema recoverable; C1 evidence binds the exact candidate digest to the tested rollback path.
...
| C0: hardened release decision | Post-beta, evidence-driven | Complete security/acceptance package, two elapsed soaks, 500-turn run, fresh-Mac recovery, and staged household trial accepted or rejected by owner |
| C1: Apache-2.0 open-source beta | After C0 | Signed C0-bound candidate, installer/rollback, CI, SBOM/notices, and reproducible public artifacts |

“Two-stage delivery” refers only to the pre-FB0 feedback program: Stage 1 is the disposable first loop and Stage 2 is the governed family-ready private beta. C0 hardening and C1 publication form the subsequent release track, not a hidden third private-beta stage.
...
1. **Home automation:** Home Assistant becomes the deterministic device plane; Tuya/Zigbee lights are integrated behind policy-controlled actions.
...
| Two-stage delivery | A disposable weeks 1–2 loop gets hardware feedback early; FB0 targets family use in weeks 6–8 while C0/C1 hardening continues on evidence rather than weakening safety gates |
