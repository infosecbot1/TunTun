...
| Tuntun/office host | The Phase 1 owner-approved opaque inventory target, currently verified as Darwin arm64, is the one physical Tuntun and office host. The family-ready baseline disconnects its BE800 link and single-homes it on the inner ASUS network by one selected Ethernet or Wi-Fi interface; architecture/model/year strings are descriptive, not authority |
...
- The Phase 1 owner-approved active Core inventory target is both the office laptop and Tuntun host. The family-ready baseline disconnects its BE800 cable and single-homes it on one inner-ASUS interface; inventory must not draw or authorize a second machine or derive authority from a model, product, year, or architecture string.
...
For this household profile, the owner-approved active Core must prove a hardware-backed, non-exportable P-256 private key through an availability and actual-signing capability probe bound to its opaque inventory target and current runtime; failure blocks P2-3/P2-4 rather than silently using a file key. No `T2`, Apple-silicon, architecture, model, product, or model-year string substitutes for that capability evidence. The integration stores only the public key, key ID, activation/revocation time, fingerprint, and controller epoch. Initial pinning and rotation require an owner passkey in Tuntun plus a separate local HA owner/admin confirmation; at most the old and new public keys overlap for 24 hours, and the old key is then rejected. The open-source key-provider port may support other hardware-backed implementations, but no lower-assurance software fallback is enabled by default.
