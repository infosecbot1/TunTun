...
| Network | ISP ONT → Archer BE800 outer router → ASUS GT-AX6000 inner router with three AX5400 AiMesh nodes. The Phase 1 family-ready baseline single-homes the Mac and Reachy on the same trusted ASUS/AiMesh L2 and disconnects the Mac's direct BE800 Ethernet link |
...
- every production build identifies the exact phase feature manifest, schemas, policy corpus, model/runtime artifacts, SBOM, provenance, and acceptance evidence; and
- every real campaign longer than one manifest's 24-hour maximum binds Phase 2's one canonical externally pre-issued `SignedFeatureManifestRolloverChainV1`, its ordered transition receipts, one frozen candidate, and zero expired-authority interval; runtime holds no acceptance signer or renewal path.
...
The Reolink video plane is process-, key-, and storage-separated from canonical identity/memory. It sends only validated event/health/presence metadata and opaque media references to policy. Music, display, TV, desktop, local inference, and robot services each have similarly narrow authority. From Phase 3, browsers reach the owner/subject APIs and exact media route only through `tuntun-owner-ingress`; non-media requests go to core UDS and the exact authorized byte-range media request goes to media-proxy UDS. Every later route-graph promotion checkpoint rebuilds that same locked ingress wheel, refreshes/re-signs the canonical Phase 3 service row against the exact signed route-manifest digest, rejects the predecessor row except with its complete matching rollback set, and reruns the installed dispatch/negative-reachability/takeover/service-lifecycle matrix before physical or elapsed evidence begins on the new graph. Phase 6 adds its VPN listener class to that same ingress process; it never makes HA, cameras, endpoints, desktop helpers, or robots remote services.
...
Phase 2 owns the only continuous feature-authority mechanism used through Phase 6: externally signed `SignedFeatureManifestV1` envelopes valid for at most 24 hours, an exact hash-linked `SignedFeatureManifestRolloverChainV1`, one immutable composition-generation swap, `FeatureManifestLeaseSupervisor`, and per-admission `FeatureAuthorityLease`. Pre-issued future envelopes cannot activate before `valid_from`; every request, preparation/commit/retry, provider call, trigger, and background iteration requires both half-open wall validity and a process-local non-extendable monotonic lease. Runtime has no acceptance signer, renewal service, fallback manifest, or grace period. A missing, late, reordered, widened, rollback, signature-invalid, candidate-drifted, or expired current/next envelope closes affected work before preparation or I/O, invalidates any uninterrupted campaign, and enters controlled whole-composition recovery. Phases 3–6 import these exact contracts and supervisor rather than creating local equivalents.
...
flowchart TB
  ...
  INGRESS <-->|same-origin protected UI| BROWSER
  INGRESS <-->|isolated subject passkey ceremony\nno owner session| SUBJECT
  INGRESS <-->|pre-session + session-bound UDS| API
  ACTION <-->|signed typed state/action/routine| GREEN
  GREEN --> MZHUB
  GREEN --> MEDIA
  GREEN --> TV
  REC -->|closed event/health only| POLICY
  API -->|authorize/register opaque short-lived capability| REC
  INGRESS -->|exact authenticated byte range only| PROXY
  POLICY --> DISPLAY
...
flowchart LR
  ...
  REMOTE[Approved owner VPN device\nPhase 6 only] -. encrypted peer route to console :8443\nand authoritative DNS :53 only .-> MAC
  PUBLIC[Public scanner] -. no forwarding / no listener .-x MAC
  BE -. direct Mac Ethernet disabled\nin family-ready baseline .-x MAC
...
- Outer-network clients receive no ambient route or listener to the inner Tuntun service. Office use on the same Mac does not create an outer-to-inner bridge; a different remote client requires the Phase 6 paired VPN/device design.
...
- The Phase 6 Tailscale `grants` policy exposes only the Mac console origin on TCP 8443 and the nonrecursive authoritative `tuntun.home.arpa` resolver on TCP/UDP 53 to the exact approved node set. No other DNS name/zone, recursive query, update, AXFR, subnet route, exit node, Funnel, public Serve, Tailscale SSH, or direct HA/video/endpoint path exists.
...
| Browser/UI | Authenticated but untrusted presenter | Versioned read models, opaque prepared IDs, short-lived media capabilities | Owner ingress network boundary; core origin/CSRF/session/passkey authority over authenticated UDS; server-built binding |
...
| Owner API and console | 1–6 | Core-owned versioned projections, exact prepared-action ceremonies, health/privacy/operations UI | UI read models, opaque prepared IDs, no canonical table exposure | Mac core + authenticated browser | Policy/auth, feature registry, all phase read services, owner ingress from Phase 3 | Same-origin, CSRF, passkey; browser untrusted; core owns no TCP listener after Phase 3 takeover | Stale/unknown shown explicitly; no client-side authority | Build React/API; adopt accessible primitives |
| Owner ingress | 3–6 | Sole HTTP listener after serialized Phase 3 takeover; forward bounded pre-session and session-bound non-media requests to core UDS and exact authorized ranges to media-proxy UDS | Listener class, normalized request commitments, fresh core-derived session tuple; no canonical authority, credential store, or video catalogue | Separate least-privilege Mac process | Core owner/subject API, media proxy, exact loopback/LAN and optional Phase 6 VPN listener classes | Authenticated peer-bound UDS; no session creation, policy decision, broad proxy, wildcard bind, or direct media authority | Admission closes on core/peer/takeover uncertainty; rollback first proves every competing listener closed | Build |
| Feature registry | 1–6 | Register only installed, accepted routes/contracts/UI modules; atomically roll one immutable composition generation under the canonical pre-issued chain | Signed feature manifest/rollover-chain state, schema/policy/evidence digests, monotonic lease | Mac core/release artifact | Release verifier and acceptance evidence; runtime has no acceptance signer | Absence removes backend and UI capability; every admission rechecks wall/monotonic authority | Missing/invalid successor, expiry, rollback, widening, or digest drift closes admission and recomposes feature-absent | Build one shared Phase 2–6 supervisor |
...
| Plugin supervisor | 6 | Verify manifest/signature and the `phase6.initial.1` registry, choose the codec out of band, launch one fresh constrained process, enforce resources and revoke | Trusted envelopes stay supervisor-side; child receives/returns only dedicated metadata-free bounded payload/render bytes | Mac isolated process/account | Release verifier and enforceable macOS sandbox | No request/grant/commitment/plugin/version/capability/codec/registry metadata, inherited secrets/core imports/network/DNS/redirects/writable mount/wildcards; result untrusted | Kill/revoke sockets/grants and erase per-call sandbox; plugin support absent if isolation is unenforceable | Build |
...
| Owner UI | Same-origin localhost HTTP or paired HTTPS; session + Origin/CSRF; WebAuthn for step-up | `ui.household_posture.v1`, `ui.prepared_action.v1`, `ui.operation_result.v1` | Browser ↔ owner ingress ↔ core API over authenticated UDS from Phase 3; Phase 1/2 core listener is removed by the serialized takeover | Read models cache only to `valid_until`; mutation uses server prepared ID + one idempotency key |
| Subject/guardian decision | Separate local same-origin subject zone; subject-bound WebAuthn; one-use opaque ceremony, no owner cookie/session | `ui.subject_self_service.v1`, `ui.guardian_exact_decision.v1`, matching prepared commitment/result | Adult/current-primary-guardian browser ↔ owner ingress ↔ Mac subject API over authenticated UDS from Phase 3 | One subject/child/resource/decision only; expiry/revoke/generation change ends it; no navigation or upgrade into owner API |
...
| Camera playback | Same-origin authenticated byte-range proxy with opaque capability | `media.playback_grant.v1` | Core media broker authorizes/registers; browser → owner ingress → media-proxy UDS for the exact range | Grant bound to session/clip/view/operation/range; short-lived and no-store |
...
| Remote access | VPN peer path to pinned HTTPS owner origin plus passkey app session | `remote_session.v1`, remote route/node state | Owner device ↔ existing owner ingress VPN listener class ↔ Mac owner API over authenticated UDS | Revocation generation terminates sessions; remote origin never upgrades authority |
...
For work spanning more than one envelope, registration authority is not a long-lived row or locally renewable lease. The externally pre-issued `SignedFeatureManifestRolloverChainV1` freezes the candidate, package, and complete registration set, hash-links each at-most-24-hour signed envelope, and requires `previous.valid_from < successor.valid_from < previous.expires_at` plus a strictly later successor expiry for one unambiguous atomic composition swap. Every transport uses the same active composition generation and `FeatureAuthorityLease`; equality with either the wall or monotonic deadline is expired. Campaign evidence records the chain ID, ordered envelope and transition-receipt digests, exact interval, and literal zero expired-authority intervals.
...
The canonical generic envelope is frozen and owned by Phase 2. Later phases import and specialize it without a local alias, field addition, or redefinition; the Phase 2 contract and generated `tuntun.home.events.v1` schema are the single field-level authority.

```python
from tuntun_contracts.home.events import CrossDomainEventV1
```

The frozen validator exact-binds `event_type` to `payload.schema_id` and envelope `event_id` to payload `event_id`; requires positive `(source_generation, source_sequence)`; bounds `observed_at <= ingested_at <= observed_at + 30 seconds` and `ingested_at < expires_at <= observed_at + 60 seconds`; and fixes `sensitivity_class="household_private_metadata"`. Payload type must match `event_type`. Device events contain no speaker identity, transcript, biometric, private memory, credential, or authorization. Registered families include:
...
```json
{
  ...
  "expires_at": "2026-08-27T12:30:59.000000Z",
  "correlation_id": "f53938a4-911b-4558-a511-fb20a65fd962",
  "causation_id": null,
  "deduplication_key": "8cd44f858cd44f858cd44f858cd44f858cd44f858cd44f858cd44f858cd44f85",
  "sensitivity_class": "household_private_metadata",
  "payload": {
    "event_id": "6aa7c33f-b72b-43f2-a722-5cd43a0f1f8c",
...
| `DesktopGrantV1` | 5 | Subject/device/level/exact roots/identities/globs/quotas/registries/`network=none`/time/policy/auth | No secret roots, symlink/mount escape, ambient shell/network |
| `DesktopCommandProposalV1` | 5 | Registered executable/argv/cwd/env/reads/writes/`network=none`/limits/state commitment | Proposal only; exact confirmation and state recheck required |
| `DesktopWorkflowManifestV1` | 5 | Signed steps/commands/mounts/artifacts/`network=none`/resources/sandbox/owner auth/expiry | Disposable sandbox only; cannot modify/commit/push live repo |
...
| `RobotMotionLeaseV1` | 5 | Robot/session/sequence/≤250 ms expiry/exact area+robot-binding+zone generations/activation commitment/safety digest/velocities/owner/controller/core-key generation/fixed signature domain | Deterministically clamped manual input only; model cannot emit; geofence state is only evaluation of the canonical zone |
| `remote_session.v1` | 6 | App actor, VPN adapter/node pseudonym, current Tailnet-Lock-signed node/set generation, passkey assurance, expiries/operations/policy/revocation | VPN membership or node signature alone grants nothing; local-only operations remain denied |
| `RemoteOperationRequestV1` / `AuthorizedRemoteContextV1` | 6 | HTTP carries only closed operation, typed opaque resource, idempotency; server resolves owner/session/node/resource/generations and signs a ≤5-second context | Browser cannot assert target, desired state, actor/session/policy/generation/time/commitment; downstream ports accept only server context |
| `AuthoritativeResolverBindingV1` | 6 | Exact LAN and Tailnet views for `tuntun.home.arpa`, current source sets/addresses/TCP+UDP53/local CA/grants digest | Authoritative only; no recursion/update/AXFR/wildcard/AAAA/cross-view cache |
...
| `UpdateRunJournalEnvelopeV1` | 6 | Canonical nonrecursive payload digest, fixed signature domain/key generation, legal state edge, sequence/prior-digest CAS, fsync facts | Startup reconciles before service exposure; corrupt/multiple/interrupted journals quarantine |
| `MacTargetReceiptV1` / `CompatibilityManifestV1` | 6 | Exact architecture/OS/final-artifact lifecycle receipts plus both public collect-only smoke architectures | Smoke/VM/synthetic evidence cannot qualify real hardware or another artifact |
| C0/C1 assertion, accepted, and publication receipts | 6 | Exact candidate/artifact set, namespace, assertion/signer/domain/local-presence window/revocation; publication is third fresh action | Household owner signs only C0; isolated maintainer signs C1/publication; household UI is C1 read-only |
