...
- **One physical Mac and one ingress server:** the active canonical Tuntun Core host is the owner-approved Darwin arm64 Mac recorded in `docs/architecture/decisions/0001-phase1-host-baseline.md`; there is no second office laptop/helper. The family-ready profile single-homes it on the inner GT-AX6000/AiMesh network with any direct BE800 link disconnected. An optional dual-home profile is a separately qualified feature with no forwarding, bridging, outer ingress, or ambient helper authority. Phase 6 extends the accepted Phase 3 owner-ingress server with listener class `owner_vpn_https`; Core and the media proxy own no TCP listener and no parallel HTTP server. Moving household deployment back to the 2020 Intel Mac requires fresh real-host qualification before it can carry live household or Tailscale evidence.
