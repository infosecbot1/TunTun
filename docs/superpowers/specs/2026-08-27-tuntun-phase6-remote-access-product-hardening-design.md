...
- **One physical Mac and one ingress server:** the inventoried 2020 Intel Mac is both the office-use machine and the only canonical Tuntun Core host; there is no second office laptop/helper. The family-ready profile single-homes it on the inner GT-AX6000/AiMesh network with the direct BE800 link disconnected. An optional dual-home profile is a separately qualified feature with no forwarding, bridging, outer ingress, or ambient helper authority. Phase 6 extends the accepted Phase 3 owner-ingress server with listener class `owner_vpn_https`; Core and the media proxy own no TCP listener and no parallel HTTP server.
...
- macOS package passes Developer ID, hardened-runtime/entitlement review, notarization, stapling, Gatekeeper, clean install, upgrade, rollback, preserve uninstall, and destroy uninstall on the supported Intel target and every current Apple Silicon architecture/OS/artifact combination claimed by the compatibility manifest. Every enabled Linux service target passes the corresponding exact-final-artifact systemd, compose or Reachy-managed-app lifecycle and residue matrix. Public runner smoke cannot substitute for a required real-target receipt.
