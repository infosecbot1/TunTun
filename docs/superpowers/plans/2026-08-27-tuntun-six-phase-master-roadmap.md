...
|---|---|
| Core host | One owner-approved Darwin arm64 Mac, normally at home and defined by ADR 0001. FileVault and owner-only local administration are required. The 2020 Intel Mac is distribution/future-transition hardware until fresh real-host qualification passes |
...
- The active Core host is the owner-approved Darwin arm64 Mac in ADR 0001. Phase 6 extends the Phase 3 owner-ingress server and cannot invent a second helper or parallel server. Moving household deployment back to the 2020 Intel Mac requires fresh real-host qualification before any POC, preflight, lifecycle, soak, or release receipt can use it.
