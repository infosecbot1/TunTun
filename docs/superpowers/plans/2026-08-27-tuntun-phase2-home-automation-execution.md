...
24. The BE800 is the outer router and the GT-AX6000 plus three AX5400 nodes are the inner household network. The stated office laptop and Tuntun Mac are the same 2020 MacBook Pro. The family-ready baseline disconnects its BE800 link and single-homes it on one inner-ASUS interface; dual-homing that same host is a separately disabled gate, never an assumed second machine. Double NAT is topology, not mutual isolation. No VLAN/SSID isolation claim is allowed without exact firmware and negative-reachability evidence.
...
def test_family_baseline_has_one_single_homed_mac(inventory, network_probe) -> None:
    assert inventory.physical_hosts.named({"office_laptop", "tuntun_mac"}) == {
        inventory.macbook_2020_host_id,
    }
    assert network_probe.enabled_interfaces(inventory.macbook_2020_host_id) == {"inner_asus"}
