...
**Architecture:** Run a narrow `tuntun-edge` service on Reachy for local wake/VAD, media, privacy/stop, and gestures. Run one ports-and-adapters modular monolith, `tuntun-core`, on the independently owner-approved opaque Core inventory target from ADR 0001, currently verified as Darwin `arm64`, for orchestration, local identity, policy/auth, canonical memory, provider routing, audit, owner API, and the React console. Architecture/model/product/year strings are descriptive and never authorize commissioning. Intel macOS remains a mandatory supported-distribution target, not the active household host unless fresh owner approval plus real-host probes requalify it. Connect Core and Reachy over a paired mTLS WebSocket protocol; use only explicit outbound cloud calls and no public inbound access.
...
- Modify `apps/core/pyproject.toml` and `uv.lock` only after the owner-approved active Core ONNX performance/correctness gate and both hosted macOS arm64/Intel distribution lock/import gates pass; the isolated conversion environment uses its own lock.
...
- Create `tests/unit/identity/test_voice_quality.py`, `test_fusion.py`, `tests/security/test_biometric_authorization.py`, `test_voice_retention.py`, `test_voice_replay_attacks.py`, `tests/acceptance/test_voice_calibration.py`, `tests/hardware/test_control_host_voice_runtime.py`, and `tests/ci/test_voice_runtime_distribution.py`.
...
- [ ] Run the stop/go Python/ONNX Runtime performance and correctness probe on the independently owner-approved active Core inventory target. Separately require install/import/numerical-smoke evidence on both hosted `macos-26` arm64 and `macos-15-intel` distribution rows; neither hosted architecture row identifies or commissions the household target. Do not place any unsupported PyTorch/Torchaudio stack in the long-lived core; if conversion/runtime/vulnerability gates fail, disable voice production and evaluate another governed encoder behind the same port.
...
- [ ] Run `uv run pytest tests/unit/identity/test_voice_quality.py tests/unit/identity/test_fusion.py tests/security/test_biometric_authorization.py tests/security/test_voice_retention.py tests/security/test_voice_replay_attacks.py tests/acceptance/test_voice_calibration.py tests/hardware/test_control_host_voice_runtime.py tests/ci/test_voice_runtime_distribution.py -q`; accept the hardware performance result only when its opaque target binding matches the trusted active-Core approval, never from architecture/model/product/year text.
