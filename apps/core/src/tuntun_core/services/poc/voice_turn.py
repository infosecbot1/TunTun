...
from .ports import CapturedTurn, VoiceAttemptAuthorizerPort
...
async def _close(iterator: object) -> None:
    ...
    *iterators: object,
    ...
    for iterator in iterators:
        ...
        try:
            ...
            if not preserve_cancellation and close_cancelled is None:
...
class VoiceTurnPipeline:
    ...
    async def _run(self, captured: CapturedTurn) -> AsyncIterator[SpeechChunk]:
        ...
        source_stream: AsyncIterator[SpeechChunk] | None = None
        converted_stream_input: AsyncIterator[bytes] | None = None
        converted_stream: AsyncIterator[bytes] | None = None
        ...
        try:
            ...
            try:
                transcription_candidate = await self._authorizer.authorize_transcription(
                    turn_id=turn_id,
            ...
            stt_cancelled = False
            try:
                transcript_candidate = await self._stt.transcribe(
                    transcription_request,
                    stt_audio,
                )
                _raise_if_cancelling()
                ...
                await _close_all(stt_audio, preserve_cancellation=stt_cancelled)
                stt_audio = None
            ...
            reasoning_candidate = await self._authorizer.authorize_reasoning(
                turn_id=turn_id,
            ...
            if not self._valid_reasoning_request(reasoning_request, turn_id=turn_id):
                raise VoiceTurnError
            response_candidate = await self._llm.complete(reasoning_request)
            _raise_if_cancelling()
            ...
            synthesis_candidate = await self._authorizer.authorize_synthesis(
                turn_id=turn_id,
                response=response,
            )
            _raise_if_cancelling()
            ...
            source_stream = self._tts.synthesize(synthesis_request)
            source_samples = await self._buffer_source(
                source_stream,
                request_id=synthesis_request.request_id,
                destination=source_pcm,
            )
            converted_stream_input = _one_chunk(source_pcm)
            conversion_cancelled = False
            try:
                converted_stream = self._converter.convert(
                    converted_stream_input,
                ...
                await _close_all(
                    *(
                        (converted_stream, converted_stream_input)
                        if converted_stream is not None
                        else (converted_stream_input,)
                    ),
    ...
    async def _buffer_source(
        ...
        *,
        ...
        try:
            ...
            await _close_all(stream, preserve_cancellation=source_cancelled)
        ...
        *,
        ...
        async for chunk in stream:
            ...
            if (
                ...
                or not chunk
                or len(chunk) > MAX_PCM_BYTES
                or len(chunk) % 2
                or len(destination) + len(chunk) > MAX_DIRECTION_BYTES
            ):
                raise VoiceTurnError
