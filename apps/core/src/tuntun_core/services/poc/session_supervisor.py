...
_EDGE_CLEANUP_KINDS = frozenset({ControlKind.STOP, ControlKind.CANCEL, ControlKind.ERROR})
...
class CorePttSessionSupervisor:
    ...
    async def run(self, turn_id: UUID) -> PttSessionOutcome:
        ...
        self._cleanup_trigger_t0: float | None = None
        self._pending_cleanup_observed_t0: float | None = None
        self._cleanup_deadline_t0: float | None = None
    ...
    def _terminal_cancel_failure(self) -> _SessionFailure:
        self._latch_requested_outcome(PttSessionOutcome.CANCELLED)
        self._latch_cleanup_trigger_t0()
        return _SessionFailure(PttSessionOutcome.CANCELLED, PttErrorReason.TURN_CANCELLED)
    ...
    async def _terminal_capture(self, total_deadline: float) -> None:
        ...
        if first is CorePttEvent.CANCEL:
            raise self._terminal_cancel_failure()
            ...
            while True:
                ...
                for frame in self._decoder.feed(chunk):
                    cleanup_trigger_t0 = (
                        ...
                        and frame.control.kind in _EDGE_CLEANUP_KINDS
                        else None
                    )
                    if cleanup_trigger_t0 is not None:
                        self._reserve_cleanup_deadline_t0(cleanup_trigger_t0)
                    ...
                    if guarded.disposition is GuardDisposition.ACCEPTED:
                        self._dispatch(
                            frame,
                            now=now,
                            cleanup_trigger_t0=cleanup_trigger_t0,
                        )
        except asyncio.CancelledError:
            if _cancellation_requested():
                raise
    ...
    def _dispatch(
        ...
        *,
        now: float,
        cleanup_trigger_t0: float | None = None,
    ) -> None:
        ...
        if kind is ControlKind.SESSION_READY:
            ...
            if not self._finalizing.is_set():
                self._set_signal(_Signal.CLEANUP, triggered_at=cleanup_trigger_t0)
    ...
    def _latch_cleanup_trigger_t0(self, triggered_at: float | None = None) -> float:
        trigger_t0 = self._cleanup_trigger_t0
        candidate_t0 = asyncio.get_running_loop().time() if triggered_at is None else triggered_at
        if trigger_t0 is None or candidate_t0 < trigger_t0:
            trigger_t0 = candidate_t0
            self._cleanup_trigger_t0 = candidate_t0
            deadline_t0 = self._cleanup_deadline_t0
            if deadline_t0 is not None and candidate_t0 < deadline_t0:
                self._cleanup_deadline_t0 = candidate_t0
                self._cleanup_loop_deadline = candidate_t0 + _CLEANUP_TOTAL_SECONDS
        return trigger_t0

    def _reserve_cleanup_deadline_t0(self, observed_at: float) -> None:
        """Tentatively cap cleanup deadlines before guard acceptance.

        This is deadline-only: a rejected or poisoned cleanup-looking frame cannot decide
        outcome or become the semantic cleanup trigger. The cap only prevents a later
        hard-stop from extending cleanup while an already-decoded STOP/CANCEL/ERROR waits
        behind guard-lock contention and may still be accepted.
        """

        pending_t0 = self._pending_cleanup_observed_t0
        if pending_t0 is None or observed_at < pending_t0:
            self._pending_cleanup_observed_t0 = observed_at

    def _cleanup_deadline_anchor_t0(self, cleanup_t0: float) -> float:
        deadline_t0 = self._cleanup_deadline_t0
        if deadline_t0 is None:
            pending_t0 = self._pending_cleanup_observed_t0
            deadline_t0 = cleanup_t0 if pending_t0 is None else min(cleanup_t0, pending_t0)
            self._cleanup_deadline_t0 = deadline_t0
        return deadline_t0
    ...
    def _set_signal(self, signal: _Signal, *, triggered_at: float | None = None) -> None:
        self._latch_cleanup_trigger_t0(triggered_at)
        self._normal_output_closed.set()
        hard_stop = signal in {
            _Signal.PEER_CLOSED,
    ...
    async def _cleanup(self, failure: _SessionFailure) -> PttSessionOutcome:
        ...
        loop = asyncio.get_running_loop()
        cleanup_t0 = self._latch_cleanup_trigger_t0()
        deadline_t0 = self._cleanup_deadline_anchor_t0(cleanup_t0)
        self._cleanup_loop_deadline = deadline_t0 + _CLEANUP_TOTAL_SECONDS
        ...
        if provider_close is not None:
            try:
                if not await self._observe_task_until(
                    provider_close,
                    deadline=deadline_t0 + _PROVIDER_CLOSE_SECONDS,
                    cancelled_ok=False,
                ):
                    cleanup_complete = False
            except BaseException as error:
        ...
        if abort_sent is not None and not await self._observe_task_until(
            abort_sent,
            deadline=deadline_t0 + _CLEANUP_ACK_SECONDS,
            cancelled_ok=False,
        ):
            cleanup_complete = False
        ...
        if self._provider_task is not None and not await self._observe_task_until(
            self._provider_task,
            deadline=deadline_t0 + _PROVIDER_JOIN_SECONDS,
            cancelled_ok=True,
        ):
            cleanup_complete = False
        ...
        if poisoned or transport_unusable:
            close_deadline = (
                self._cleanup_loop_deadline
                if self._pending_cleanup_observed_t0 is not None
                else None
            )
            await self._close_bridge_once(deadline=close_deadline)
        ...
        ack_deadline = deadline_t0 + _CLEANUP_ACK_SECONDS
        try:
        ...
        if not cleanup_complete:
            return PttSessionOutcome.CLEANUP_INCOMPLETE
        try:
            guarded_outcome = self._guard.finish()
        except FrameProtocolError:
            return PttSessionOutcome.CLEANUP_INCOMPLETE
        if self._cleanup_outcome_latched:
