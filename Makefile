...
core-wheel-smoke:
	@set -eu; base="$${TMPDIR:-/tmp}"; case "$$base" in /*) ;; *) exit 97 ;; esac; while [ "$$base" != / ] && [ "$${base%/}" != "$$base" ]; do base="$${base%/}"; done; prefix="$${base%/}/tuntun-core-wheel."; smoke="$$(mktemp -d "$${prefix}XXXXXX")"; cleanup() { case "$$smoke" in "$$prefix"*) rm -rf -- "$$smoke" ;; *) exit 97 ;; esac; }; trap cleanup 0; export UV_CACHE_DIR="$$smoke/uv-cache"; uv build --package tuntun-contracts --wheel --out-dir "$$smoke/dist"; uv build --package tuntun-core --out-dir "$$smoke/dist"; set -- "$$smoke"/dist/tuntun_core-*.whl; [ "$$#" -eq 1 ] && [ -f "$$1" ]; core_wheel="$$1"; uv venv --python 3.12 "$$smoke/venv"; uv pip install --resolution highest --find-links "$$smoke/dist" --python "$$smoke/venv/bin/python" "$$core_wheel"; env -u PYTHONPATH "$$smoke/venv/bin/python" -c 'import importlib.util as u; from importlib.metadata import requires, version; from uuid import UUID; assert u.find_spec("tuntun_testing") is None; from typer.testing import CliRunner; from tuntun_core.cli.main import app; import tuntun_core.cli.commands.simulate; from tuntun_core.adapters.sqlcipher.crypto import RecordCipher, RecordContext; assert "tuntun-contracts==0.1.0.dev0" in (requires("tuntun-core") or []); assert version("tuntun-contracts") == "0.1.0.dev0"; context = RecordContext(household_id=UUID(int=1), table="biometric_templates", row_id=UUID(int=2), purpose="voice-template", schema_version="1.0"); cipher = RecordCipher(bytes(range(32))); encrypted = cipher.encrypt(b"isolated-wheel-record-sentinel", context); assert cipher.decrypt(encrypted, context) == b"isolated-wheel-record-sentinel"; result = CliRunner().invoke(app, ["version"]); assert result.exit_code == 0 and result.stdout == "0.1.0.dev0\n"; missing = CliRunner().invoke(app, ["simulate", "--scenario", "missing.yaml"]); assert missing.exit_code == 2 and missing.stdout == "" and missing.stderr == "simulation-extra-required\n"; models = CliRunner().invoke(app, ["models", "list"]); assert models.exit_code == 0 and models.stdout == "[]\n"'
